 ...
 *
 */
public class SmallMasterDao extends Dao {
	/**
	 * コンストラクタ。
	 * @param obj JDBC接続可能オブジェクト。
	 * @throws Exception 例外。
	 */
	public SmallMasterDao(final JDBCConnectableObject obj) throws Exception {
		super(obj);
	}
	 ...
	 */
	public static class SmallMasterTableQuery extends Query {
		/**
		 * コンストラクタ。
		 */
		public SmallMasterTableQuery() {
	...
	public static FieldList getQueryResultFieldList() {
		Query query = new SmallMasterTableQuery();
	...
	public int insert(final Map<String, Object> data) throws Exception {
		return this.executeInsert(new SmallMasterTable(), data);
	}
...
//		return this.executeRemove(new SmallMasterTable(), data); // レコードの論理削除(DeleteFlagの設定)
		return this.executeDelete(new SmallMasterTable(), data); // レコードの物理削除
	}
}
